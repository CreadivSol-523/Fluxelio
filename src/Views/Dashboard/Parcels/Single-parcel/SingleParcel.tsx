                  ...
                  <div className="grid grid-cols-4 gap-4 max-2xl:grid-cols-3 max-xl:grid-cols-2 max-sm:grid-cols-1 max-sm:gap-2" style={{ paddingLeft: "0.75rem", paddingRight: "0.75rem" }}>
                     <div className="flex flex-col gap-1 w-full">
                        ...
                        {singleParcel?.packagePicture?.length == 0 ? (
                           ...
                           <div className="flex flex-col gap-1 w-full">
                              {/* <Image src={`${activeImage}`} alt="img" width={350} height={150} className="w-full h-[150px] object-cover rounded-md" /> */}
                              <div className="flex items-center gap-1 overflow-x-auto">
                                 {singleParcel?.packagePicture?.map((item, index) => (
                                    <>
                                       {console.log(item?.includes("uploads") ? `${apiUrl}/${item}` : `${item}`, "itemitemitemitem")}
                                       <img key={index} src={item?.includes("uploads") ? `${apiUrl}/${item}` : `${item}`} alt="img" width={150} height={50} className="w-[32.5%] h-[70px] object-cover rounded-md cursor-pointer" onClick={() => setActiveImage(item)} />
                                    </>
                                 ))}
                              </div>
                           </div>
