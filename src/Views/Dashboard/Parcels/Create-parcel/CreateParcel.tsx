...
const CreateParcel = () => {
   ...
   const agencyID = role === "Admin" ? params.AgencyID : role === "Agency" ? cookiesData?.data?.user?._id : cookiesData?.data?.user?.agencyID;
   ...
   const handleSubmit = async () => {
      await handleCreateParcel({
         createdBy: cookiesData?.data?.user?._id,
         agencyID: agencyID,
         officeID: data.officeID,
         data: { ...data, tagID: data.mixedPackage ? data.tagID : null, notificationCost: data.whatsappNotif ? data.notificationCost || null : null, partialAmount: data.partialAmount || null },
         Token: Token,
      });
            ...
            <div className="flex flex-col gap-6 px-4 py-3 dark:bg-cardBG rounded-sm bg-layoutLightBG  dark:border-textColorDark border-2 border-transparent w-full">
               ...
               <div className="grid grid-cols-4 gap-6 max-lg:grid-cols-3 max-sm:grid-cols-1">
                  ...
                  <div className="flex flex-col gap-1 h-12 max-sm:mt-4">
                     ...
                     <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                           <Button variant="outline" className=" border dark:border-textColorDark border-textColorLight w-full h-full bg-transparent" size="lg" disabled={!data.officeID}>
                  ...
                  <div className="flex flex-col gap-1 h-12 max-sm:mt-4">
                     ...
                     <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                           <Button variant="outline" className=" border dark:border-textColorDark border-textColorLight w-full h-full bg-transparent" size="lg" disabled={!data.officeID || !data.mixedPackage}>
                              {data.tagName ? data.tagName : "Select Tag"}
                           </Button>
                        </DropdownMenuTrigger>
                        <DropdownMenuContent align="start">
                           {selectedOfficesTags?.data?.tags?.map((item) => (
                              <DropdownMenuItem key={item?._id} onClick={() => handleDropdown({ name: "tagName", value: item?.tagName, id: item?._id })}>
                                 {item?.tagName}
                              </DropdownMenuItem>
                           ))}
                        </DropdownMenuContent>
                  ...
                  <div className="flex flex-col gap-1 mt-4">
                     ...
                     <Field placeHolder="Estimate Arrival" type="text" value={data.estimateArrival} name="estimateArrival" onChange={handleData} />
                  </div>
                  <div className="flex flex-col gap-1 mt-4 max-lg:mt-0">
                     <label>Weight</label>
                     <Field placeHolder="Weight in kg" type="number" value={data.weight} name="weight" onChange={handleData} />
                  </div>
                  <div className="flex flex-col gap-1 mt-4 max-lg:mt-0">
                     <label>Price Per kg</label>
                     <Field placeHolder="Price Per kg" type="number" value={data.pricePerKilo} name="pricePerKilo" onChange={handleData} />
                  </div>
                  <div className="flex flex-col gap-1">
                     <label>Actual Carrier Cost</label>
                     <Field placeHolder="Actual Carrier Cost" type="number" value={data.actualCarrierCost} name="actualCarrierCost" onChange={handleData} />
                  </div>
                  <div className="flex flex-col gap-1 h-12">
                     <label>Status</label>
                     <DropdownMenu>
                        ...
                        <DropdownMenuContent align="start">
                           {allStatus.map((item, index) => (
                              <DropdownMenuItem key={`status${index}`} onClick={() => handleDropdown({ name: "status", value: item })}>
                                 {item}
                              </DropdownMenuItem>
                           ))}
                        </DropdownMenuContent>
                     ...
                     <DropdownMenu>
                        ...
                        <DropdownMenuContent align="start">
                           {allPaymentStatus.map((item, index) => (
                              <DropdownMenuItem key={`paymentStatus${index}`} onClick={() => handleDropdown({ name: "paymentStatus", value: item })}>
                                 {item}
                              </DropdownMenuItem>
                           ))}
                        </DropdownMenuContent>
                  ...
                  <div className="flex flex-col gap-1 max-sm:mt-4">
                     ...
                     <Field placeHolder="Partial Amount" type="number" value={data.partialAmount} name="partialAmount" onChange={handleData} disabled={data.paymentStatus == "PENDING PAYMENT"} />
                  </div>
                  <div className="flex flex-col gap-2">
                     <div className="flex gap-2 max-sm:mt-4">
                        <label>Mixed Parcel?</label>
                        <div className="flex mt-1">
                           <Switch id="airplane-mode" className="border border-textColorLight darK:border-textColorDark" value={data.mixedPackage} onCheckedChange={(checked) => setData({ ...data, mixedPackage: checked, tagID: checked ? data.tagID : "" })} />
                        </div>
                     </div>
                  </div>
                  <div className="flex flex-col gap-2">
                     <div className="flex gap-2 ">
                        ...
                        <div className="flex mt-1">
                           <Switch id="airplane-mode" className="border border-textColorLight darK:border-textColorDark" value={data.whatsappNotif} onCheckedChange={(checked) => setData({ ...data, whatsappNotif: checked, notificationCost: checked ? data.notificationCost : "" })} />
                        </div>
                     </div>
                     {data.whatsappNotif && <Field placeHolder="Notification Cost" type="number" value={data.notificationCost} name="notificationCost" onChange={handleData} />}
                  </div>
                  <div className="flex flex-col gap-1 col-span-2 max-sm:col-span-1">
                     <label>Parcel Description</label>
                     <Field placeHolder="Parcel Description" type="textarea" row={6} value={data.description} name="description" onChange={handleData} />
                  </div>
               </div>
               <div className="flex items-center justify-center gap-4 sm:flex-row flex-col">
                  <CustomButton name="Create Parcel" pClass="font-semibold" className="border border-BtnColorLight" onClick={handleSubmit} isLoading={isLoading} disabled={isLoading} />
                  <CustomButton isDark={false} name="Back" pClass="font-semibold" className="border dark:border-white border-gray-400 w-[8rem]" />
               </div>
            </div>
         </div>
      </Layout>
